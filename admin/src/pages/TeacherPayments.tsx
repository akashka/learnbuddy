...
export default function TeacherPayments() {
  ...
  const [tab, setTab] = useState<'make' | 'history'>('make');
    ...
    <div className="p-8">
                  ...
                  <tbody>
                    {calcResult.breakdown.map((b, i) => (
                      <tr key={i} className="border-t border-accent-100">
                        ...
                        <td className="px-4 py-2 text-right">₹{b.feePerMonth}</td>
                      ...
                      {history.payments.map((p) => (
                        <tr key={p._id} className="border-t border-accent-100">
                          ...
                          <td className="px-4 py-2 text-right">
                            {p.commissionAmount != null && p.tdsAmount != null ? `₹${(p.commissionAmount + p.tdsAmount).toFixed(2)}` : '-'}
                          </td>
                          ...
                          <td className="px-4 py-2">{p.paidAt ? new Date(p.paidAt).toLocaleString() : '-'}</td>
                          <td className="px-4 py-2">
                            <button
                              type="button"
      ...
      {receiptId && receipt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setReceiptId(null)}>
          <div
            className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-accent-200 bg-white p-6 shadow-lg"
          ...
          >
            ...
            <div className="space-y-4 print:block">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <p><strong>Paid at:</strong> {receipt.paidAt ? new Date(receipt.paidAt).toLocaleString() : '-'}</p>
              ...
              <div className="border-t border-accent-200 pt-4 text-sm">
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span>Gross earnings</span>
                    <span>₹{(receipt.grossAmount ?? receipt.amount).toFixed(2)}</span>
                  </div>
                  {receipt.commissionAmount != null && (
                    <div className="flex justify-between text-amber-700">
                      <span>Platform commission ({receipt.commissionPercent ?? 0}%)</span>
                      <span>- ₹{receipt.commissionAmount.toFixed(2)}</span>
                    </div>
                  )}
