 ...
 * Free tier: $5 credits for new accounts. gpt-4o-mini has vision.
 */
...
const TEXT_MODEL = 'gpt-4o-mini'; // Vision + text, cost-effective
const VISION_MODEL = 'gpt-4o-mini'; // Supports images
...
export async function openaiGenerateWithImageAndAudio(
  ...
  if (audioDataUrl?.startsWith('data:')) {
    content.push({ type: 'text', text: '[Audio transcript not available - analyze image only. If audio was provided, assume it may contain speech.]' });
  }
  const res = await client.chat.completions.create({
    model: VISION_MODEL,
    messages: [{ role: 'user', content }],
