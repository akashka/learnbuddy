...
export function ParentDashboardScreen() {
  ...
  if (error) return <View style={styles.center}><InlineErrorDisplay error={error} onRetry={fetchProfile} /></View>;

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>{t('dashboard')}</Text>
      <Text style={styles.welcome}>{t('welcome')}, {profile?.name || profile?.email || 'Parent'}!</Text>
      <Card style={styles.card}>
        ...
        <Text style={styles.row}><Text style={styles.bold}>Location:</Text> {profile?.location || '-'}</Text>
      </Card>
      <TouchableOpacity
        style={styles.privacyCard}
        onPress={() => navigation.navigate('ParentPrivacy')}
      >
        <Text style={styles.privacyIcon}>🔒</Text>
        <View style={styles.privacyText}>
          <Text style={styles.privacyTitle}>Privacy & Data</Text>
          <Text style={styles.privacyHint}>View data, consent history, download or delete</Text>
        </View>
        <Text style={styles.privacyArrow}>→</Text>
      </TouchableOpacity>
      <View style={styles.actions}>
        <TouchableOpacity style={styles.btn} onPress={() => navigation.navigate('ParentMarketplace')}>
          <Text style={styles.btnText}>{t('marketplace')}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.btn} onPress={() => navigation.navigate('ParentStudents')}>
          <Text style={styles.btnText}>{t('myKids')}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.btn} onPress={() => navigation.navigate('ParentClasses')}>
